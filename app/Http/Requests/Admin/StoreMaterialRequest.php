...
class StoreMaterialRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'source_type' => ['required', 'in:uploaded_pdf,external_link,video'],
        ...
        // Conditional validation per source type
        if ($sourceType === 'uploaded_pdf') {
            $rules['book_file'] = ['required', 'file', 'mimes:pdf', "max:{$maxFileSize}"];
        } elseif ($sourceType === 'external_link') {
            $rules['external_url'] = [
                'required',
                'string',
                'max:2048',
    ...
    public function messages(): array
    {
        return [
            ...
            'source_type.required' => 'Pilih sumber materi.',
            'source_type.in' => 'Sumber materi tidak valid.',
            'book_file.required' => 'Berkas PDF e-book wajib diunggah untuk sumber upload PDF.',
            'book_file.mimes' => 'Format berkas harus berupa PDF.',
            'book_file.max' => 'Ukuran berkas PDF melebihi batas maksimal (50 MB).',
            'external_url.required' => 'URL tautan buku digital wajib diisi.',
            'video_url.required' => 'URL video pembelajaran wajib diisi.',
            'cover_file.image' => 'Cover harus berupa berkas gambar yang valid (JPG, PNG, WebP).',
