    ...
    return (
        <div className={`w-full bg-white rounded-xl border border-[#DCE7F3] shadow-xs overflow-hidden ${className}`}>
            ...
            <div className="overflow-x-auto min-w-full">
                <table className="w-full text-left border-collapse text-xs" aria-label={ariaLabel}>
                    <thead>
                    ...
                    <tbody className="divide-y divide-[#DCE7F3]/60 bg-white">
                        {data.length === 0 ? (
                            <tr>
                                <td colSpan={columns.length} className="p-8">
                                    <EmptyState
                                        ...
                                        actionText={emptyActionText || actionText}
                                    />
                                </td>
                            </tr>
                        ) : (
                            data.map((row, rowIndex) => (
                                <tr
                                    key={typeof rowKey === 'function' ? rowKey(row, rowIndex) : row[rowKey] || rowIndex}
                                    className="hover:bg-[#F8FBFF] transition-colors motion-reduce:transition-none group"
                                >
